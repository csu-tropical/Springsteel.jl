...
# ═══════════════════════════════════════════════════════════════════════════
# 3D Cylindrical Transforms  (Spline×Fourier×Spline = RLR)
# ═══════════════════════════════════════════════════════════════════════════
#
# Same spectral layout as RLZ — z-major × wavenumber-interleaved (TRAP-1
# convention p = (k-1)*2). The k direction uses CubicBSpline rather than
# Chebyshev: physical layout has kDim = nc_k * mubar mish points (not
# Chebyshev–Lobatto collocation). b_kDim = nc_k + 3 spline B-coefficients.

const _RLRGrid = SpringsteelGrid{CylindricalGeometry, SplineBasisArray{2}, FourierBasisArray{2}, SplineBasisArray{1}}

function getGridpoints(grid::_RLRGrid)
    iDim = grid.params.iDim
    kDim = grid.params.kDim
    jDim = grid.params.jDim
    gridpts = zeros(Float64, kDim * jDim, 3)
...
num_columns(grid::_RLRGrid) = grid.params.jDim

function spectralTransform!(grid::_RLRGrid)
    _filter_mish!(grid)
    spectralTransform(grid, grid.physical, grid.spectral)
    applyFilter!(grid)
    return grid.spectral
...
function spectralTransform(grid::_RLRGrid, physical::Array{real}, spectral::Array{real})
    ...
    # tempsb[z_b, l] holds the k-direction spline B-coefficients per (r, l)
    # for one r at a time, mirroring tempcb in the RLZ transform.
    tempsb = zeros(Float64, b_kDim, 4 + 4 * kDim_wn)
...
function gridTransform!(grid::_RLRGrid)
    gridTransform(grid, grid.physical, grid.spectral)
    return grid.physical
end

function gridTransform(grid::_RLRGrid, physical::Array{real}, spectral::Array{real})
    ...
    ringBuffer     = zeros(Float64, 4 + 4 * kDim_wn, b_kDim)
    ...
    for v in 1:size(spectral, 2)
        kspl = grid.kbasis.data[v]
        for dr in 0:2
            # ── i-direction Spline + FAtransform stage ────────────────────────
            for z_b in 1:b_kDim
                r1 = (z_b - 1) * b_iDim * (1 + kDim_wn * 2) + 1
                r2 = r1 + b_iDim - 1
                z_slot_base = (z_b - 1) * slots_per_z
            ...
            # ── Fourier + k-Spline inverse stage ──────────────────────────────
            zi = 1
            for r in 1:iDim
                ri      = r + grid.params.patchOffsetL
                lpoints = 4 + 4*ri
...
# ── Regular-grid output — 3D Cylindrical (RLR) ─────────────────────────────

function getRegularGridpoints(grid::_RLRGrid)
    n_r   = grid.params.i_regular_out
    n_λ   = grid.params.j_regular_out
    n_z   = grid.params.k_regular_out
    r_pts = collect(LinRange(grid.params.iMin, grid.params.iMax, n_r))
...
function regularGridTransform(grid::_RLRGrid, r_pts::AbstractVector{Float64},
                               λ_pts::AbstractVector{Float64}, z_pts::AbstractVector{Float64})
    gp       = grid.params
    kDim_wn  = gp.iDim + gp.patchOffsetL
    b_iDim   = gp.b_iDim
...
function regularGridTransform(grid::_RLRGrid, gridpoints::AbstractMatrix{Float64})
    r_pts = sort(unique(gridpoints[:, 1]))
    λ_pts = sort(unique(gridpoints[:, 2]))
    z_pts = sort(unique(gridpoints[:, 3]))
    return regularGridTransform(grid, r_pts, λ_pts, z_pts)
