...
function _create_tile_from_patch(patch::SpringsteelGrid,
    ...
    tile_gp = SpringsteelGridParameters(
        ...
        # Spline j/k axes are untouched by i-tiling, so inherit the patch's
        # already-resolved cell counts rather than re-deriving them from the
        # tile's narrower i-domain. (0 for Fourier/Chebyshev axes, which is the
        # "unset" sentinel and leaves their own derivation alone.)
        num_cells_j    = patch.params.num_cells_j,
        num_cells_k    = patch.params.num_cells_k,
        ...
        # Compose the patch's own physical offset (nonzero for nest annulus
        # patches whose ring numbering / wavenumber support follows the GLOBAL
        # grid) with the tile's position inside the patch. spectralIndexL stays
        # patch-relative — it windows the tile into the patch spectral array —
        # while patchOffsetL carries the global mish offset. For ordinary
        # patches (patchOffsetL == 0) this reduces to the previous derivation.
        patchOffsetL   = patch.params.patchOffsetL +
                         (spectralIndexL - 1) * patch.params.mubar,
        tile_num       = tile_num)
...
# 3D Cylindrical Spline×Fourier×{Chebyshev or Spline}  (RLZ / RLR)
# Buffer shape: (iDim, 3, b_kDim, nvars) — 3 columns per k coefficient level for k=0, k≥1 real, k≥1 imag.
function allocateSplineBuffer(tile::SpringsteelGrid{CylindricalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}})
    return zeros(Float64, tile.params.iDim, 3, tile.params.b_kDim, length(tile.params.vars))
end

# 3D Spherical Spline×Fourier×{Chebyshev or Spline}  (SLZ / SLR) — identical layout to RLZ.
function allocateSplineBuffer(tile::SpringsteelGrid{SphericalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}})
    return zeros(Float64, tile.params.iDim, 3, tile.params.b_kDim, length(tile.params.vars))
end
...
function calcPatchMap(patch::SpringsteelGrid{CylindricalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}},
                       tile::SpringsteelGrid{CylindricalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}})
    n           = size(patch.spectral, 1)
    nvars       = size(patch.spectral, 2)
    kDim        = tile.params.iDim + tile.params.patchOffsetL
...
function calcPatchMap(patch::SpringsteelGrid{SphericalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}},
                       tile::SpringsteelGrid{SphericalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}})
    n           = size(patch.spectral, 1)
    nvars       = size(patch.spectral, 2)
    kDim        = tile.params.iDim + tile.params.patchOffsetL
    b_kDim      = tile.params.b_kDim
...
function calcHaloMap(patch::SpringsteelGrid{CylindricalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}},
                      tile1::SpringsteelGrid{CylindricalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}},
                      tile2::SpringsteelGrid{CylindricalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}})
    n           = size(patch.spectral, 1)
    nvars       = size(patch.spectral, 2)
    kDim        = tile1.params.iDim + tile1.params.patchOffsetL
...
# 3D Spherical (SLZ) — identical z-major / wavenumber layout to RLZ.
function calcHaloMap(patch::SpringsteelGrid{SphericalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}},
                      tile1::SpringsteelGrid{SphericalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}},
                      tile2::SpringsteelGrid{SphericalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}})
    n           = size(patch.spectral, 1)
    nvars       = size(patch.spectral, 2)
    kDim        = tile1.params.iDim + tile1.params.patchOffsetL
    b_kDim      = tile1.params.b_kDim
...
function sumSpectralTile!(patch::SpringsteelGrid{CylindricalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}},
                           tile::SpringsteelGrid{CylindricalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}})
    kDim        = tile.params.iDim + tile.params.patchOffsetL
    b_kDim      = tile.params.b_kDim
    siL         = tile.params.spectralIndexL
...
# 3D Spherical (SLZ) — identical z-major / wavenumber layout to RLZ.
function sumSpectralTile!(patch::SpringsteelGrid{SphericalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}},
                           tile::SpringsteelGrid{SphericalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}})
    kDim        = tile.params.iDim + tile.params.patchOffsetL
    b_kDim      = tile.params.b_kDim
    siL         = tile.params.spectralIndexL
...
function setSpectralTile!(patch::SpringsteelGrid{CylindricalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}},
                           tile::SpringsteelGrid{CylindricalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}})
    patch.spectral[:] .= 0.0
    kDim        = tile.params.iDim + tile.params.patchOffsetL
    b_kDim      = tile.params.b_kDim
...
# 3D Spherical (SLZ) — identical z-major / wavenumber layout to RLZ.
function setSpectralTile!(patch::SpringsteelGrid{SphericalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}},
                           tile::SpringsteelGrid{SphericalGeometry, <:SplineBasisArray, <:FourierBasisArray, <:Union{ChebyshevBasisArray, SplineBasisArray}})
    patch.spectral[:] .= 0.0
    kDim        = tile.params.iDim + tile.params.patchOffsetL
    b_kDim      = tile.params.b_kDim
...
function splineTransform!(sharedSpectral::SharedArray{real}, tile::_RLGrid)
    ...
    # Reload the coupled per-wavenumber border coefficients before every solve
    # on the reused splines (see the 3-arg method and gridTransform).
    ...
    for v in 1:nvars
        ...
        k2 = b_iDim
        if has_wn_ahat
            tile.ibasis.data[1, v].ahat .= _get_wavenumber_ahat(tile, v, 0)
        end
        ...
        for k in 1:kDim
            ...
            p2 = p * b_iDim
            if has_wn_ahat
                tile.ibasis.data[2, v].ahat .= _get_wavenumber_ahat(tile, v, p)
            end
            ...
            p2 = (p + 1) * b_iDim
            if has_wn_ahat
                tile.ibasis.data[3, v].ahat .= _get_wavenumber_ahat(tile, v, p + 1)
            end
...
# ────────────────────────────────────────────────────────────────────────────
# splineTransform! / tileTransform!  (B → A) — 3D Cylindrical (RLR)
# ────────────────────────────────────────────────────────────────────────────
#
# Same spectral layout as RLZ, so the SA solve walks identical (z_b, k) blocks
# and uses the same wavenumber-offset convention.

function splineTransform!(sharedSpectral::SharedArray{real}, tile::_RLRGrid)
    b_iDim  = tile.params.b_iDim
    b_kDim  = tile.params.b_kDim
    kDim_wn = tile.params.iDim + tile.params.patchOffsetL
    nvars   = length(tile.params.vars)
            ...
            for r in 1:iDim
                ...
                for dl in 0:2
                    ...
                    for l in 1:lpoints
                        ...
                        if dr == 0 && dl == 0
                            SItransform!(kspl)
                            physical[z1:z2, v, 1] .= kspl.uMish
                            SIxtransform(kspl, kspline_scratch)
                            physical[z1:z2, v, 6] .= kspline_scratch
                            SIxxtransform(kspl, kspline_scratch)
                            physical[z1:z2, v, 7] .= kspline_scratch
                        elseif dr == 0 && dl == 1
                            SItransform!(kspl)
                            physical[z1:z2, v, 4] .= kspl.uMish
                        elseif dr == 0 && dl == 2
                            SItransform!(kspl)
                            physical[z1:z2, v, 5] .= kspl.uMish
                        elseif dr == 1
                            SItransform!(kspl)
                            physical[z1:z2, v, 2] .= kspl.uMish
                        elseif dr == 2
                            SItransform!(kspl)
                            physical[z1:z2, v, 3] .= kspl.uMish
                        end
                    end
                end  # for dl
...
# ────────────────────────────────────────────────────────────────────────────
# splineTransform! / tileTransform!  (B → A) — 3D Spherical (SLR)
# ────────────────────────────────────────────────────────────────────────────

function splineTransform!(sharedSpectral::SharedArray{real}, tile::_SLRGrid)
    b_iDim  = tile.params.b_iDim
    b_kDim  = tile.params.b_kDim
    kDim_wn = tile.params.iDim + tile.params.patchOffsetL
    nvars   = length(tile.params.vars)
...
function tileTransform!(sharedSpectral::SharedArray{real},
    ...
    for v in values(tile.params.vars)
        ...
        for dr in 0:2
            ...
            zi = 1
            for r in 1:iDim
                lpoints    = tile.jbasis.data[r, 1].params.yDim
                ringBuffer = zeros(Float64, lpoints, b_kDim)
...
# ════════════════════════════════════════════════════════════════════════════
# 3-argument splineTransform!  (multi-tile B → A)
#
# The 2-argument splineTransform!(sharedSpectral, tile) applies the TILE's
# SA matrix to convert B-coefficients → A-coefficients.  This is correct
# only when tile = patch (single-tile mode), because the tile's SA matrix
# does not match the patch-level B-coefficients in the shared spectral.
#
# These 3-argument methods use the PATCH's SA matrix on the full shared
# spectral, then extract the tile's portion of A-coefficients.  This is the
# correct approach for multi-tile distributed workflows where the tiles
# are subdomains of the patch.
#
# After this call, tile.spectral contains valid A-coefficients for
# tileTransform!.  patch.spectral is used as a scratch buffer.
# ════════════════════════════════════════════════════════════════════════════

"""
    splineTransform!(sharedSpectral, patch, tile) -> Nothing

Multi-tile variant: apply the SA transform using `patch`'s splines on the
full `sharedSpectral`, then extract the tile's portion of A-coefficients into
`tile.spectral`.

Use this form in distributed workflows where `tile` is a subdomain of `patch`.
The 2-argument form `splineTransform!(sharedSpectral, tile)` remains valid for
single-tile (tile = patch) scenarios.

# Arguments
- `sharedSpectral::SharedArray{Float64}`: patch-sized shared spectral (B-coefficients).
- `patch`: full-domain grid whose `ibasis.data` splines provide the SA matrix.
- `tile`: tile grid whose `spectral` array receives the extracted A-coefficients.

See also: [`tileTransform!`](@ref)
"""
function splineTransform!(sharedSpectral::SharedArray{real},
                            patch::_1DCartesianGrid,
                            tile::_1DCartesianGrid)
    siL    = tile.params.spectralIndexL
    b_iDim = tile.params.b_iDim

    for v in 1:length(tile.params.vars)
        patch.spectral[:, v] .= SAtransform(patch.ibasis.data[1, v],
                                             view(sharedSpectral, :, v))
        tile.spectral[1:b_iDim, v] .= patch.spectral[siL:siL+b_iDim-1, v]
    end
    return nothing
end
...
function splineTransform!(sharedSpectral::SharedArray{real},
    ...
    # Multi-patch coupling: the reused splines (k0/real/imag) are shared across
    # wavenumbers, so the coupled border coefficients must be reloaded from the
    # per-wavenumber registry before EVERY SAtransform, exactly as the non-tiled
    # gridTransform does — otherwise every k >= 1 solve reuses whatever ahat the
    # last apply_interface_payload! left behind.
    has_wn_ahat = _has_wavenumber_ahat(patch)

    for v in 1:nvars
        # k = 0 block
        if has_wn_ahat
            patch.ibasis.data[1, v].ahat .= _get_wavenumber_ahat(patch, v, 0)
        end
        patch.spectral[1:b_iDim_patch, v] .=
            SAtransform(patch.ibasis.data[1, v],
                        view(sharedSpectral, 1:b_iDim_patch, v))
        tile.spectral[1:b_iDim_tile, v] .=
            patch.spectral[siL:siL+b_iDim_tile-1, v]

        # k >= 1: real and imaginary blocks
        for k in 1:kDim
            p = k * 2

            # Real part
            pp1 = (p - 1) * b_iDim_patch + 1
            tp1 = (p - 1) * b_iDim_tile + 1
            if has_wn_ahat
                patch.ibasis.data[2, v].ahat .= _get_wavenumber_ahat(patch, v, p)
            end
            patch.spectral[pp1:pp1+b_iDim_patch-1, v] .=
            ...
            # Imaginary part
            pp1 = p * b_iDim_patch + 1
            tp1 = p * b_iDim_tile + 1
            if has_wn_ahat
                patch.ibasis.data[3, v].ahat .= _get_wavenumber_ahat(patch, v, p + 1)
            end
            patch.spectral[pp1:pp1+b_iDim_patch-1, v] .=
